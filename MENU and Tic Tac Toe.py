...
class  ArcadeSystem(tk.Tk):                                                                     # initializing the app
    def __init__(self, *args, **kwargs):                                                                                            #I DON'T KNOW WHAT TO COMMENT ON THIS CLASS. 
        ...
        for F in (StartPage, Page1, LoginPage, NewAccPage, PlayPage, AreYouSure, EditPage, EditUsername, EUsedName, UsernameChanged, EditPassword, PasswordChanged, NoMatch, AccSuccessful, AccFailed, UsedName, InvalidDOB, LoginFailed, LoginSuccess, LoggedOut, AccountDeleted): 
...
##        Ulabel.pack()
    ...
    def LogOut(self):
        ...
        app.show_frame(LoggedOut)

class LoggedOut(tk.Frame):                                                                       # Frame that is raised if the username is already taken
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)

        self['bg']='light blue'
        
        Label = tk.Label(self, text="You have been logged out.", font=SMALL_FONT, bg="light blue", fg="indigo", activeforeground="black").pack()                 # Creating and placing label displaying appropriate message

        NewAcc_Button = tk.Button(self, text="Back to Main Menu", font=SMALL_FONT, bg="green", fg="white", activeforeground="black",                                    # button that raises the NewAcc frame so that the user can change the incorrect credentails
                            command=lambda: controller.show_frame(StartPage))
...
#        Username_File.close()
    ...
    def LogOut(self):
        os.remove("LoggedInAccount.dat")
        os.remove("LoggedInUser.txt")
        app.show_frame(AccountDeleted)
        
class AccountDeleted(tk.Frame):                                                                       # Frame that is raised if the username is already taken
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)

        self['bg']='light blue'
        
        Label = tk.Label(self, text="This Account has been deleted.", font=SMALL_FONT, bg="light blue", fg="indigo", activeforeground="black").pack()                 # Creating and placing label displaying appropriate message

        NewAcc_Button = tk.Button(self, text="Return to Main Menu", font=SMALL_FONT, bg="green", fg="white", activeforeground="black",                                    # button that raises the NewAcc frame so that the user can change the incorrect credentails
                            command=lambda: controller.show_frame(StartPage))
        NewAcc_Button.place(x=100,y=200)                                                        # placing the button
...
class EditPassword(tk.Frame):
    ...
    def EPCheck(self):
        ...
        if EPassword_Entry.get() != ReEPassword_Entry.get():
            ...
            app.show_frame(PasswordChanged)
